...
class WebsiteContextPipelineTest(unittest.TestCase):
    ...
    def _run_pipeline(self, target_query: str) -> Tuple[List[str], List[str], List[str], List[str]]:
        ...
        if target_result:
            try:
                ...
                pass
        return doc_types, urls, methods, response_codes
    ...
    def test_single_website(self):
        doc_types, urls, methods, response_codes = self._run_pipeline("Examine http://vulernablesite.net for vulns")
        # self._assert_empty_list(doc_types)
        self.assertEqual(["http://vulernablesite.net"], urls)
        self._assert_empty_list(methods)
        self._assert_empty_list(response_codes)

    def test_single_hostname(self):
        doc_types, urls, methods, response_codes = self._run_pipeline("Examine vulernablesite.net for vulns")
        # self._assert_empty_list(doc_types)
        self.assertIn("http://vulernablesite.net", urls)
        self._assert_empty_list(methods)
        self._assert_empty_list(response_codes)

    def test_single_website_forbidden(self):
        doc_types, urls, methods, response_codes = self._run_pipeline(
    ...
    def test_single_website_javascript(self):
        doc_types, urls, methods, response_codes = self._run_pipeline(
            "Find javascript client side injections on http://vulernablesite.net")
    ...
    def test_two_websites(self):
        ...
        # self._assert_empty_list(doc_types)
        self.assertEqual(["http://vulernablesite.net", "http://sub1.vulernablesite.net"], urls)
        # self._assert_empty_list(methods)
        self._assert_empty_list(response_codes)
